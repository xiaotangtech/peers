        ...
        while (!isStopped) {
            ...
            try {
                ...
                if (payloadType == RFC3551.PAYLOAD_TYPE_G729) {
                     ...
                     if (result.length == process_size) {
                         rawData.write(encoder.process(result));
                         rawData.flush();
                         result = new byte[0];
                     }
                     if (result.length > process_size) {
                         byte[] temp = new byte[process_size];
                         System.arraycopy(result, 0, temp, 0, process_size);
                         rawData.write(encoder.process(temp));
                         rawData.flush();
                         byte[] temp2 = new byte[result.length - process_size];
                         System.arraycopy(result, process_size, temp2, 0, result.length - process_size);
                         result = new byte[0];
